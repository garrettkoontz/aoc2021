...
class Day17 : Day<Day17.TargetArea, Int, Int> {
    ...
    class Probe(val velocity: Point, val position: Point = Point(0, 0)) {
        fun next(): Probe =
            Probe(velocity.decayVelocity(), position + velocity)
    ...
    override fun part1(input: TargetArea): Int {
        val hits = (0..1000).flatMap { x -> (-1000..1000).mapNotNull { y -> Probe(Point(x, y)).hitsTargetArea(input) } }
        return hits.maxOf { it }
    }
